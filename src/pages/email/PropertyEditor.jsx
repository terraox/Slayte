...
function TextInput({ value, onChange, placeholder, multiline }) {
  const cls = "w-full bg-card border border-border rounded-lg text-[13px] text-muted-foreground px-3 py-2 focus:outline-none focus:border-[#7c3aed]/50 transition-colors resize-none";
  return multiline
...
function NumberInput({ value, onChange, min, max }) {
  return (
    <input
      ...
      className="w-full bg-card border border-border rounded-lg text-[13px] text-muted-foreground px-3 py-2 focus:outline-none focus:border-[#7c3aed]/50 transition-colors"
    />
  );
}
...
function ColorInput({ value, onChange }) {
  return (
    <div className="flex items-center gap-2">
      <div className="relative w-9 h-9 rounded-lg border border-border overflow-hidden flex-shrink-0 cursor-pointer">
        <input type="color" value={value} onChange={e => onChange(e.target.value)}
          className="absolute inset-0 w-[200%] h-[200%] -top-1/2 -left-1/2 cursor-pointer opacity-0" />
        <div className="absolute inset-0" style={{ backgroundColor: value }} />
      </div>
      <input value={value} onChange={e => onChange(e.target.value)}
        className="flex-1 bg-card border border-border rounded-lg text-[13px] text-muted-foreground font-mono px-3 py-2 focus:outline-none focus:border-[#7c3aed]/50 transition-colors" />
    </div>
  );
}
        ...
        <button
          ...
          className={`flex-1 flex flex-col items-center gap-1 py-2.5 rounded-xl border transition-all duration-150 ${
            value === a
              ...
              : 'border-border bg-card text-muted-foreground hover:text-muted-foreground hover:border-border hover:bg-card'
          }`}
        >
    ...
    switch (type) {
        ...
        return <>
          ...
          <Field label="Font Weight">
            <select value={p.fontWeight} onChange={e => set('fontWeight', e.target.value)}
              className="w-full bg-card border border-border rounded-lg text-[13px] text-muted-foreground px-3 py-2 focus:outline-none">
      ...
      case 'social':
        return <>
          <Field label="Social Links">
            <p className="text-[11px] text-muted-foreground mb-1.5 leading-snug">One per line — format: <span className="font-mono text-[#5DCAA5]">Platform|URL</span></p>
      ...
      case 'html':
        return <Field label="Raw HTML">
          <p className="text-[11px] text-muted-foreground mb-1.5">Paste any valid HTML. Inline styles recommended for email.</p>
      ...
      case 'icons':
        return <>
          <Field label="Icons">
            <p className="text-[11px] text-muted-foreground mb-1.5">Comma-separated emoji, e.g. <span className="font-mono text-[#5DCAA5]">⭐, 🔥, ✅</span></p>
      ...
      case 'menu':
        return <>
          <Field label="Navigation Links">
            <p className="text-[11px] text-muted-foreground mb-1.5 leading-snug">One per line — format: <span className="font-mono text-[#5DCAA5]">Label|URL</span></p>
      ...
      case 'table':
        return <>
          <Field label="Column Headers">
            <p className="text-[11px] text-muted-foreground mb-1.5">Comma-separated, e.g. <span className="font-mono text-[#5DCAA5]">Name, Email, Role</span></p>
      ...
      default:
        return <p className="text-muted-foreground text-[13px]">No editable properties.</p>;
    }
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border flex-shrink-0">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1">Properties</p>
          <h3 className="text-[15px] font-semibold text-foreground capitalize tracking-tight">{type}</h3>
        </div>
